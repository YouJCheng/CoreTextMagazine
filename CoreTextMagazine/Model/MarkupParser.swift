...
class MarkupParser: NSObject {
    ...
    func parseMarkup(_ markup: String) {
        ...
        do {
            ...
            //3 Look through the string until you find an opening bracket, then look through the string until you hit a closing bracket
                ...
                if tag.hasPrefix("font") {
                    ...
                    colorRegex.enumerateMatches(in: tag,
                                                ...
                                                range: NSMakeRange(0, tag.count)) { (match, _, _) in
                                                    /*  If enumerateMatches(in:options:range:using:) returns a valid match with a valid range in tag, find the indicated value (ex. <font color="red"> returns "red") and append "Color" to form a UIColor selector. Perform that selector then set your class's color to the returned color if it exists, to black if not.*/                                                 if let match = match,
